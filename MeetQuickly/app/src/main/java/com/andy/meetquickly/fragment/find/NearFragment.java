...
public class NearFragment extends BaseLazyFragment {
    ...
    public void onEventMainThread(screenMomentMessage message) {
        screenSex = message.getSex();
        refreshData();
    }
    ...
    private void initData() {
        ...
        MQApi.getUserNearDynamicList(mPage, Uid, sex,
                RxSPTool.getContent(NearFragment.this.getContext(), Constant.KEY_LONGITUDE),
                RxSPTool.getContent(NearFragment.this.getContext(), Constant.KEY_LATITUDE), screenSex, new SimpleCallBack<String>() {
                    @Override
...
//                        JSON.parseObject(s, new TypeReference<MyApiResult<List<MomnetListBean>>>(){});
    ...
    private void initView() {
        recyclerView.setLayoutManager(new LinearLayoutManager(NearFragment.this.getContext()));
        adapter = new FindNearAdapter(mData, NearFragment.this.getContext());
        //adapter设置点击事件
        adapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                MomentDetailActivity.start(NearFragment.this.getContext(), mData.get(position).getId());
            }
        });
        adapter.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                if (view.getId() == R.id.iv_more) {
                    ...
                    UserDataActivity.start(NearFragment.this.getContext(), mData.get(position).getUId());
                    ...
                    if ("1".equals(mData.get(position).getType())) {
                        ...
                        ImagePreview.getInstance()
                                .setContext(NearFragment.this.getContext())
                                .setIndex(0)
                                .setImageList(lists)
                        ...
                        SimpleVideoPlayActivity.start(NearFragment.this.getContext(), mData.get(position).getUserDynamicFiles().get(0).getUrl());
                    }
                }
            }
        });
...
//                        JSON.parseObject(s,new MyApiResult<String>().getClass());
    ...
    private void showMoreDialog(int position) {
        customDialog = CustomDialog.show(NearFragment.this.getActivity(), R.layout.dialog_add_reserve, new CustomDialog.BindView() {
            @Override
...
//                        JSON.parseObject(s, new TypeReference<MyApiResult<List<ReportBean>>>() {});
    ...
    private void shieldMoment(int position) {
        showWaitDialog(NearFragment.this.getActivity());
    ...
    private void showReportDialog(int position) {
        reportContent = "";
        //活动
        customDialog = CustomDialog.show(NearFragment.this.getContext(), R.layout.dialog_report, new CustomDialog.BindView() {
            @Override
            public void onBind(CustomDialog dialog, View rootView) {
                ////绑定布局
                //ImageView btnOk = rootView.findViewById(R.id.btn_ok);
                RecyclerView recyclerView = rootView.findViewById(R.id.recycler_view);
                LinearLayoutManager layoutManager = new LinearLayoutManager(NearFragment.this.getContext());
