...
public class FlowLayout extends ViewGroup {
    ...
    private List<UserBaseBean> allUrls = new ArrayList<>();
...
//            Collections.reverse(lineViews);
    ...
    public void setUrls(List<UserBaseBean> listVals) {
        allUrls.addAll(listVals);
        //开始绘制view
        setViews();
    }

    public void setOneUrls(UserBaseBean urlVal) {
        allUrls.add(urlVal);
        setViews();
    }

    public void cancels(UserBaseBean urlVal) {
    ...
    private void setViews() {
        ...
        for (int i = 0; i < allUrls.size(); i++) {
            ...
            ImageLoaderUtil.getInstance().loadCircleImage(context,allUrls.get(i).getFace(),imageView);
            int finalI = i;
            imageView.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    UserDataActivity.start(context,allUrls.get(finalI).getUid());
                }
            });
